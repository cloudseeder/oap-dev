...
**Vector search as a cold-start dependency.** The OAP reference discovery architecture uses a vector database (ChromaDB with nomic-embed-text embeddings) to match natural language tasks to manifest descriptions. This creates an apparent tension with the procedural memory thesis: if small models struggle with the multi-step reasoning needed to evaluate vector search results, the very mechanism that feeds candidates to the experience-building loop may itself be unreliable for small models.

This tension is real but self-resolving by design. During cold start, the experience cache is empty and the agent depends entirely on vector search + LLM ranking — the same pipeline a standard OAP agent uses. At this stage, the small model may make errors: selecting wrong manifests, misinterpreting parameter requirements, or failing to construct valid invocations. Some of these attempts will nonetheless succeed, and those successes populate the experience cache.

As the cache warms, the agent progressively bypasses vector search. Path 1 (cache hit) skips discovery entirely, resolving tasks from proven patterns. Path 2 (partial match) uses cached experience as a strong prior, reducing the evaluation burden on the small model from open-ended candidate ranking to binary confirmation of a known-good pattern. The vector database becomes a fallback for genuinely novel tasks rather than the primary resolution mechanism.

This implies that the early interactions of a cold-start small-model agent will have lower success rates than a frontier model performing the same discovery. The critical empirical question is whether enough early interactions succeed to bootstrap the cache past the tipping point where cache-based resolution dominates. If the vector search + small LLM pipeline produces even a 40-50% success rate on first attempts, the experience cache should converge to near-frontier performance within a modest number of interactions. The experimental evaluation (Section 4) is designed to measure this convergence curve directly.

It also suggests a practical deployment strategy: **warm-start seeding**. An operator could pre-populate the experience cache with records generated by a frontier model during an initial calibration phase, then hand off to the small model for ongoing operation. This eliminates the cold-start vulnerability entirely, at the cost of a one-time frontier model invocation per task pattern.
