...
**Caveat: tool correctness ≠ answer correctness.** The claim "correct answer guaranteed" holds only when the tool produces the complete answer. For multi-step problems — where the tool computes a partial result and the model must interpret it — `think: false` prevents verification. A PhD-level biology question (circular plasmid digestion with EcoRI) exposed this: the model summed fragment sizes as 4.5 kb instead of 4.7 kb, then confidently reported a discrepancy that didn't exist. The tool (`bc`) was never called with the addition — the model attempted the arithmetic itself in its summarization step, with thinking suppressed. The fix is **conditional thinking**: a configurable `think_prefixes` list gates `think: true` on fingerprint prefix. When a task fingerprints as `compute.*`, the model gets a thinking budget to verify tool output before responding. Non-compute queries stay on the fast `think: false` path. The overhead is ~2 seconds — a 12× speedup over raw thinking with equivalent correctness. See "The Model That Knew Tomorrow" (Section: The Fix — Conditional Thinking) for the full comparison.
