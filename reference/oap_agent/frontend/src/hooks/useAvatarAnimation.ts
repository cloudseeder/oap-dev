    ...
    function tick(now: number) {
      ...
      switch (mode) {
        case 'idle':
          ...
          glowRadius = 0.6 + 0.05 * Math.sin(t * style.idleSpeed * 1.5)
          glowAlpha = 0.12 + 0.04 * Math.sin(t * style.idleSpeed * 1.5)
          break
        case 'speaking': {
          ...
          glowRadius = 0.7 + 0.15 * base
          glowAlpha = 0.2 + 0.15 * base
          break
        }
        case 'listening':
          scale = 1.05 + 0.02 * Math.sin(t * 3)
          glowRadius = 0.75 + 0.05 * Math.sin(t * 2)
          glowAlpha = 0.2 + 0.05 * Math.sin(t * 2)
          break
        case 'thinking':
          scale = 1 + 0.04 * Math.sin(t * 2)
          rotation = t * 1.5
          glowRadius = 0.6 + 0.08 * Math.sin(t * 3)
          glowAlpha = 0.1 + 0.08 * Math.abs(Math.sin(t * 1.5))
          break
      }
