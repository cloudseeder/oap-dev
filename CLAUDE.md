...
- **Discovery test harness**: Automated test script covering ~200 discovery and execution patterns across diverse tools. Should test semantic matching quality (does the right tool surface?), invocation correctness (does the LLM construct valid args?), and execution success (does the tool produce expected output?). Include negative feedback detection: when a tool execution fails (error output, non-zero exit), log the failure, skip caching, and retry with the next-best candidate. The negative feedback loop in `tool_api.py` already handles error gating and confidence degradation — the test harness should exercise these paths end-to-end.
