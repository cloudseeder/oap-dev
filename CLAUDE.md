...
- Key files: `models.py` (Pydantic types), `validate.py` (validation), `crawler.py`, `db.py` (ChromaDB), `discovery.py` (vector search + LLM), `api.py` (FastAPI), `ollama_client.py` (Ollama API client — `generate()` for `/api/generate` with optional `think` param, `chat()` for `/api/chat`), `config.py` (configuration), `cli.py` (CLI entry point)
...
- **Discovery test harness**: Automated test script covering ~200 discovery and execution patterns across diverse tools. Should test semantic matching quality (does the right tool surface?), invocation correctness (does the LLM construct valid args?), and execution success (does the tool produce expected output?). Include negative feedback detection: when a tool execution fails (error output, non-zero exit), log the failure, skip caching, and retry with the next-best candidate. This feeds into the negative feedback loop below.
- **Experience negative feedback loop**: The `ExperienceRecord` model already has a `corrections` list (`CorrectionEntry` with `attempted`, `error`, `fix` fields) but it's not wired up. The procedural memory paper (`docs/OAP-PROCEDURAL-MEMORY-PAPER.md`) defines three-path routing (3a/3b/3c) and correction chains but doesn't specify what happens when a cached result *fails* — there's no 3a-fail path that downgrades confidence and falls through to 3b/3c. Needed at both spec and implementation levels: (1) detect bad results (empty output, error codes, user rejection), (2) populate corrections and downgrade confidence on the cached entry, (3) evict or skip low-confidence entries so discovery falls through to full vector search + LLM ranking. Without this, a bad cache entry (e.g. `findrule` cached for `search.system.command_lookup`) persists forever and keeps getting served.
