...
- **Discovery test harness** (implemented): `scripts/discovery-test-harness.py` — 200 integration tests across all 7 local manifests (grep, wc, jq, date, bc, apropos, man) plus cross-tool and negative tests. Tests semantic matching quality, invocation correctness, and execution success. Tiered verdicts (PASS/SOFT/WARN/FAIL/SKIP) handle LLM non-determinism. CLI: `--category`, `--test`, `--smoke`, `--dry-run`, `--fail-fast`, `--verbose`, `--json`. Cache tests behind `--include-cache-tests --token <secret>` exercise miss→hit flow via experience API. Target: >80% PASS+SOFT aggregate per category. ~200 × ~8s avg ≈ 27 min sequential (Ollama serializes inference).
