...
- **Manifest factory**: `scripts/manifest-factory.py` — auto-generates OAP manifests from man pages via qwen3t:4b. Discovers section-1 tools via `apropos`, filters by path allowlist (matches `invoker.py`), blocklist (dangerous/interactive/privileged tools), and existing manifests, reads man pages via `man <tool> | col -b` (truncated to 2500 chars for 4096 context window), generates manifests using few-shot examples (grep, wc, date) with `format="json"`, `temperature=0`, `think=False`, validates output, and writes to `reference/oap_discovery/manifests/`. Self-contained: stdlib + httpx only (inline validation mirroring `validate.py` to avoid pydantic import chain). CLI: `--dry-run` (preview without Ollama), `--tools sed,awk,cut` (specific tools), `--ollama-url` (override endpoint). On Debian: ~1580 commands discovered, ~1235 after filtering, ~3s per tool via Ollama.
