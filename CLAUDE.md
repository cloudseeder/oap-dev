...
- **Experience negative feedback loop**: The `ExperienceRecord` model already has a `corrections` list (`CorrectionEntry` with `attempted`, `error`, `fix` fields) but it's not wired up. Needed: (1) detect bad results (empty output, error codes, user rejection), (2) populate corrections and downgrade confidence on the cached entry, (3) evict or skip low-confidence entries so discovery falls through to full vector search + LLM ranking. Without this, a bad cache entry (e.g. `findrule` cached for `search.system.command_lookup`) persists forever and keeps getting served.
