...
- **Cloudflare Tunnel**: Three hostnames (`api.oap.dev`, `trust.oap.dev`, `dashboard.oap.dev`) routing to local services. Discovery tunnel should only expose `/v1/discover`, `/v1/manifests`, `/health` — tool bridge routes (`/v1/chat`, `/v1/tools`, `/api/chat`), Ollama pass-through routes (`/api/tags`, `/api/show`, `/api/ps`, `/api/generate`, `/api/embed`), and experience routes (`/v1/experience`) stay local-only on `:8300`
...
- **Auth model**: Backend token auth (`X-Backend-Token` / `OAP_BACKEND_SECRET`) is per-route, not global. Protected routes: `/v1/discover`, `/v1/manifests`, `/v1/manifests/{domain}`, `/health`, `/v1/experience/*`. Unprotected routes: `/v1/chat`, `/v1/tools`, `/api/chat`, `/api/tags`, `/api/show`, `/api/ps`, `/api/generate`, `/api/embed`, `/api/embeddings` (local-only, secured by tunnel path filtering)
