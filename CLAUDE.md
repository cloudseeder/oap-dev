...
- Experience hints (`_build_experience_hints`): on subsequent requests, `_build_experience_hints(fingerprint)` queries both exact fingerprint failures (`find_failures_by_fingerprint`) and prefix-matched failures/successes (`find_failures_by_prefix`, `find_successes_by_prefix` on `ExperienceStore`) using the first two fingerprint segments (e.g., `extract.json` from `extract.json.field_list`). Returns a compact hint string injected into the system prompt: `"\n\nIMPORTANT — learning from previous attempts at this type of task:\n- oap_grep({"args":"[-Ei]..."}) → Error: invalid option — instead try: oap_grep(...)\n- Previously succeeded: local/jq for similar task\nUse successful approaches. Avoid failed ones."` Also returns a list of successful tool names for potential injection into discovery results. This teaches the LLM what not to repeat and what has worked before for similar tasks. Fingerprinting is always performed when the experience engine is available (even with `--no-cache`), so experience hints are looked up regardless of cache mode. The pre-computed fingerprint is passed to `_check_experience_cache()` to avoid redundant LLM calls.
