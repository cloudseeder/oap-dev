...
- Ollama tool bridge (enabled by default via `tool_bridge.enabled: true`): `tool_models.py` (Pydantic types for Ollama tool schema), `tool_converter.py` (manifest-to-tool conversion with heuristic parameter schema generation), `tool_executor.py` (tool call execution via `invoke_manifest()`, direct CLI execution via `execute_exec_call()`, map-reduce summarization for large results), `tool_api.py` (FastAPI router at `/v1/tools`, `/v1/chat`, and `/api/chat`). `POST /v1/tools` discovers manifests and returns Ollama tool definitions. `POST /v1/chat` and `POST /api/chat` are identical — transparent Ollama proxy that discovers tools, injects them, executes tool calls, and loops up to `max_rounds`. The `/api/chat` alias makes the OAP server a drop-in Ollama replacement (`OLLAMA_HOST=http://localhost:8300 ollama run qwen3:8b`). **Streaming**: when `stream: true` (default for standard Ollama clients like `ollama run`), the tool bridge processes everything non-streaming internally (tool loops need full responses), then wraps the final result in Ollama NDJSON format — a content chunk with `done: false` followed by a `done: true` sentinel with timing metrics. **Ollama pass-through proxy** (`api.py`): non-chat `/api/*` endpoints (`GET /api/tags`, `POST /api/show`, `GET /api/ps`, `POST /api/generate`, `POST /api/embed`, `POST /api/embeddings`) proxy directly to the real Ollama server via `_proxy_to_ollama()`, streaming response bytes through. This lets Ollama clients list models, check status, and generate text without knowing OAP exists. Tool bridge routes (`/v1/chat`, `/v1/tools`, `/api/chat`) have **no backend token auth** — they are local-only, secured at infrastructure layer by Cloudflare Tunnel path filtering (tunnel only exposes `/v1/discover`, `/v1/manifests`, `/health`). **Chat system prompt**: `tool_api.py` prepends a system message to all chat rounds with: "NEVER answer without calling a tool", API tool preference for web/API data ("use the discovered API tool — do NOT try to curl APIs via oap_exec"), oap_exec preference for CLI tasks, pipe examples (`grep -oE regex /path/file | sort -u`), jq select/length examples, `grep -c` for counting, extraction+uniqueness patterns, and inline-text stdin guidance. Combined with `think: false` in the Ollama payload, this keeps qwen3:8b responses to ~12 tokens per round. The system prompt is preserved across retry attempts (cache degradation path). **Multi-tool injection**: `_discover_tools()` injects up to `MAX_INJECTED_TOOLS = 3` tools into each chat round — the LLM's top pick from ranking plus the next highest-scoring candidates from vector search (deduped by domain). This gives the chat model alternatives when vector search ranks the wrong manifest first (e.g., "pull out lines with email addresses" pulling spfquery above grep). The chat model is smart enough to select the right tool from multiple options.
...
- **OpenAPI tool server** (enabled when `tool_bridge.enabled: true`): `openapi_server.py` (FastAPI router at `/v1/openapi.json` and `/v1/tools/call/{tool_name}`). Turns OAP into a standard OpenAPI 3.1 tool server consumable by Open WebUI, LangChain, or any OpenAPI-aware client. `GET /v1/openapi.json` dynamically generates an OpenAPI 3.1.0 spec from all indexed manifests — iterates `ManifestStore.list_domains()`, converts each via `manifest_to_tool()`, and builds path entries with JSON Schema request bodies. Includes `oap_exec` as a built-in tool. Unlike the chat flow (which suppresses stdio tools), the OpenAPI server exposes **all** manifests — the consuming client handles tool selection. `POST /v1/tools/call/{tool_name}` accepts a JSON body (the tool's arguments) and delegates to `execute_tool_call()` or `execute_exec_call()` — same security (PATH allowlist, SSRF protection), credential injection, and summarization as the chat flow. No auth (local-only, same as `/v1/chat`). State wired in `api.py` lifespan alongside tool bridge: `_store`, `_ollama`, `_tool_bridge_cfg`, `_credentials`.
...
- **Seed domain crawling on startup**: `api.py` lifespan crawls remote domains from `seeds.txt` after indexing local manifests. Uses the `Crawler` class (`crawler.py`) to fetch `https://<domain>/.well-known/oap.json` for each seed, validate, embed, and upsert into ChromaDB + FTS. Errors are logged but don't block startup. This ensures HTTP manifests (e.g. `mynewscast.org`) are discoverable alongside local CLI tools without requiring a separate `oap-crawl` run. Seeds file: `reference/oap_discovery/seeds.txt`.
...
- **Ollama compat demo**: `scripts/demo-ollama-compat.sh [oap_url]` — tests all `/api/*` endpoints (tags, ps, show, chat streaming/non-streaming, generate) to verify the OAP server works as a drop-in Ollama replacement. Default URL: `http://localhost:8300`.
...
- Key files: `config.py` (AgentConfig dataclass + YAML loader + URL validation), `db.py` (SQLite schema + CRUD: conversations, messages, tasks, task_runs — WAL mode, foreign keys, thread-safe writes via `threading.Lock`), `executor.py` (HTTP executor calling `/v1/chat` on discovery service), `scheduler.py` (APScheduler 3.x `AsyncIOScheduler` with `CronTrigger`, dynamic job management), `events.py` (in-memory EventBus: `asyncio.Queue` per SSE subscriber, 50 subscriber cap), `api.py` (FastAPI app: all routes, Pydantic request models with `max_length` + model allowlist + cron validation, SSE streaming, lifespan, StaticFiles mount for SPA)
- **Frontend** (`frontend/`): Vite 6 + React 19 + React Router 7 + Tailwind CSS 4 SPA. Built output committed to `oap_agent/static/` so the Mini needs no Node runtime. Dev: `cd frontend && npm run dev` (Vite at :5173, proxies `/v1/agent` to :8303). Build: `npm run build` outputs to `../oap_agent/static/`. Components: `AgentLayout`, `AgentSidebar`, `ChatView`, `ChatMessage`, `ChatInput`, `AgentEventProvider`, `TaskList`, `TaskDetail`, `TaskForm`, `TaskRunDetail`, `ToolCallCard`, `ExperienceBadge`, `CronInput`. Types in `src/lib/types.ts`. All API calls go to `/v1/agent/*` directly (no proxy layer).
- SPA routes: `/` (redirect to `/chat`), `/chat` (new conversation), `/chat/:id` (existing), `/tasks` (list + create), `/tasks/:id` (detail + run history)
